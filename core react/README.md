...
This is a minimal React setup using CDN links instead of a build tool like Vite or Create React App. The goal is to understand how React works under the hood by rendering elements manually with React.createElement.

📁 Project Structure react-baby-steps/ │ ├── index.html ├── App.js ├── style.css └── README.md
...
JavaScript (App.js) const head = React.createElement( "div", { id: "heading" }, "Hello world from react" );
const root = ReactDOM.createRoot(document.getElementById("root")); root.render(head);
...
Vanilla JS Comparison (Commented Out) // const head = document.createElement("div"); // head.innerHTML = "Hello world from js"; // document.getElementById("root").appendChild(head);
